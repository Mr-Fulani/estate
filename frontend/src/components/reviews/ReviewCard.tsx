...
export function ReviewCard({ review, locale }: { review: PublicReview; locale: Locale }) {
  ...
  return (
    <article className="relative flex h-full min-w-0 max-w-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 md:p-8">
      ...
      <div className="mb-5 flex items-center gap-1 pe-10 sm:pe-12" aria-label={`${review.rating} / 5`}>
      ...
      <p dir="auto" className="relative z-10 min-w-0 max-w-full flex-1 break-words text-base italic leading-7 text-slate-700 [overflow-wrap:anywhere]">&ldquo;{review.content}&rdquo;</p>
      {review.company_response && (
        <div className="mt-5 min-w-0 max-w-full rounded-2xl border border-primary/10 bg-primary/5 p-4">
          ...
          <p dir="auto" className="break-words text-sm leading-relaxed text-slate-700 [overflow-wrap:anywhere]">{review.company_response}</p>
        </div>
      )}
      <footer className="mt-6 flex min-w-0 items-start gap-3 border-t border-slate-100 pt-5">
        ...
        <div className="min-w-0 flex-1">
          <p dir="auto" className="break-words font-bold leading-5 text-slate-900 [overflow-wrap:anywhere]">{review.reviewer_name}</p>
          <p dir="auto" className="mt-0.5 break-words text-sm leading-5 text-slate-500 [overflow-wrap:anywhere]">{review.reviewer_role || review.property_title || ''}</p>
