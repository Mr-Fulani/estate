  ...
  return (
    <form id="leave-review" onSubmit={handleSubmit} className="min-w-0 max-w-full space-y-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 md:p-8">
      ...
      <div className="grid min-w-0 gap-4 sm:grid-cols-2">
        <label className="min-w-0 text-sm font-semibold text-slate-700">{copy.name}<input name="reviewer_name" required minLength={2} maxLength={100} defaultValue={initialName} autoComplete="name" className="mt-2 h-11 min-w-0 w-full max-w-full rounded-xl border border-slate-200 bg-slate-50 px-4 font-normal outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/10" /></label>
        <label className="min-w-0 text-sm font-semibold text-slate-700">{copy.role}<input name="reviewer_role" maxLength={160} placeholder={locale === 'ru' ? 'Покупатель квартиры' : locale === 'tr' ? 'Daire alıcısı' : 'Apartment buyer'} className="mt-2 h-11 min-w-0 w-full max-w-full rounded-xl border border-slate-200 bg-slate-50 px-4 font-normal outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/10" /></label>
        {!token && <><label className="min-w-0 text-sm font-semibold text-slate-700">{copy.phone}<input name="phone" type="tel" autoComplete="tel" maxLength={30} className="mt-2 h-11 min-w-0 w-full max-w-full rounded-xl border border-slate-200 bg-slate-50 px-4 font-normal outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/10" /></label><label className="min-w-0 text-sm font-semibold text-slate-700">{copy.email}<input name="email" type="email" autoComplete="email" className="mt-2 h-11 min-w-0 w-full max-w-full rounded-xl border border-slate-200 bg-slate-50 px-4 font-normal outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/10" /></label></>}
      </div>
      ...
      <label className="block min-w-0 text-sm font-semibold text-slate-700">{copy.content}<textarea name="content" required minLength={10} maxLength={5000} rows={6} className="mt-2 min-w-0 w-full max-w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-4 font-normal leading-relaxed outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/10" /></label>
