...
export function Brand({ compact = false }: { compact?: boolean }) {
  ...
  return <span translate="no">{compact ? brandInitials(settings) : <><span className="md:hidden">{brandInitials(settings)}</span><span className="hidden md:inline">{brandName(settings)}</span></>}</span>;
}
