    ...
    <details className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-6 border-t border-slate-100 bg-slate-50/60 p-5">
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          {translations.map((translation) => <fieldset key={translation.locale} dir={translation.locale === 'ar' ? 'rtl' : 'ltr'} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5"><legend className="px-2 text-sm font-bold text-primary">{translation.locale.toUpperCase()}</legend><label className="block text-xs font-bold uppercase tracking-wider text-slate-600">Роль<input value={translation.reviewer_role || ''} onChange={(event) => updateTranslation(translation.locale, 'reviewer_role', event.target.value)} className="mt-2 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-primary" /></label><label className="block text-xs font-bold uppercase tracking-wider text-slate-600">Отзыв<textarea rows={6} value={translation.content} onChange={(event) => updateTranslation(translation.locale, 'content', event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 p-3 text-sm font-normal leading-relaxed normal-case tracking-normal outline-none focus:border-primary" /></label><label className="block text-xs font-bold uppercase tracking-wider text-slate-600">Ответ Компания<textarea rows={3} value={translation.company_response || ''} onChange={(event) => updateTranslation(translation.locale, 'company_response', event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 p-3 text-sm font-normal leading-relaxed normal-case tracking-normal outline-none focus:border-primary" /></label></fieldset>)}
        </div>
